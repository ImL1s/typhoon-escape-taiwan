...
<style>
  ...
  * {{ box-sizing: border-box; -webkit-tap-highlight-color: transparent; }}
  ...
  @media (max-width: 480px) {{
    ...
    .hud-badge {{ padding: 4px 8px; font-size: 11.5px; gap: 4px; border-radius: 6px; }}
    .hud-date {{ font-size: 12px; }}
    .hud-sub {{ font-size: 10.5px; }}
    .top-btns {{ gap: 5px; right: 8px; top: 8px; }}
    .ctrl-btn {{ width: 36px; height: 36px; font-size: 15px; border-radius: 8px; }}
    .ctrl-btn svg {{ width: 17px; height: 17px; }}
  }}
  @media (max-width: 390px) {{
    .hud-date {{ font-size: 11px; }}
    .hud-sub {{ display: none; }}
    .hud-badge {{ padding: 3px 6px; font-size: 10px; }}
    .top-btns {{ gap: 4px; right: 6px; top: 6px; }}
    .ctrl-btn {{ width: 33px; height: 33px; font-size: 14px; }}
  ...
  #popup {{
    ...
    width: min(88vw, 360px); max-height: 90vh; overflow-y: auto;
  ...
  #pausemodal {{
    ...
    box-shadow: 0 16px 40px rgba(0,0,0,0.7), 0 0 0 1px rgba(0,210,255,0.25);
    backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px);
    z-index: 60; transition: all 0.25s ease;
  }}
  #pausemodal.hidden {{ display: none; }}
  #pausemodal .logo-icon {{
  ...
  #pausemodal .subhead {{
    font-size: 13.5px; color: #8DB9E8; margin-bottom: 18px; line-height: 1.5;
  }}
  #popup .logo-icon {{
...
<div id="wrap">
  ...
  <div id="pausemodal" class="hidden">
    ...
    <h2>遊戲暫停中</h2>
    <div class="subhead" id="pause-stats">台灣島暫時拋錨停泊</div>
    
    <div class="pause-actions">
      <button id="resumebtn" class="btn-primary" style="margin-bottom: 10px;">▶️ 繼續守護台灣</button>
      <button id="pause-menubtn" class="btn-copy" style="margin-bottom: 10px;">📖 查看遊戲指南與防衛機制</button>
    </div>
...
function setPaused(state) {{
  ...
  if (paused) {{
    ...
    var pstats = document.getElementById('pause-stats');
    if (pstats) pstats.textContent = '已堅守 ' + days + ' 天，成功閃避 ' + evadedCount + ' 個颱風';
    if (pauseModal) pauseModal.className = '';
    showToast('遊戲已暫停');
  }} else {{
    if (pauseModal) pauseModal.className = 'hidden';
    if (soundEnabled) startAmbientWind();
...
var pauseMenuBtn = document.getElementById('pause-menubtn');
if (pauseMenuBtn) {{
  pauseMenuBtn.onclick = function() {{
    if (pauseModal) pauseModal.className = 'hidden';
...
document.getElementById('menuclose').onclick = function() {{
  ...
  if (running && !over && paused && pauseModal) {{
    pauseModal.className = '';
  }}
}};
...
wrapEl.addEventListener('pointerdown', function(e) {{
  if (over || e.target.closest('#popup') || e.target.closest('#pausemodal') || e.target.closest('.top-btns') || e.target.closest('#menu') || active) return;
  initAudio();
...
// Keyboard Controls (WASD, Arrows, Space, P, Esc)
var keys = {{}};
function keyVel() {{
